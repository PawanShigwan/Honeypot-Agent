...
if agent_reply:
    ...
    history.append({"sender": "scammer", "text": scam_msg, "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())})
    history.append({"sender": "agent", "text": agent_reply, "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())})
else:
